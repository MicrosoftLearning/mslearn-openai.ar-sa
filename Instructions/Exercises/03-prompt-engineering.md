...
إذا لم يكن لديك واحداً بالفعل، فقم بتوفير مورد Azure OpenAI في اشتراك Azure الخاص بك.

1. سجّل الدخول إلى **Azure portal** من `https://portal.azure.com`.
2. إنشاء مورد **Azure OpenAI** بالإعدادات التالية:
    - **اشتراك**: *حدد اشتراك Azure الذي تمت الموافقة عليه للوصول إلى خدمة Azure OpenAI*
...
يوفر Azure OpenAI مدخلاً مستنداً إلى الويب يسمى **Azure OpenAI Studio**، يمكنك استخدامه لنشر النماذج وإدارتها واستكشافها. ستبدأ استكشافك لـ Azure OpenAI باستخدام Azure OpenAI Studio لنشر نموذج.

1. في صفحة **نظرة عامة** لمورد Azure OpenAI، استخدم الزر **انتقل إلى Azure OpenAI Studio** لفتح Azure OpenAI Studio في علامة تبويب متصفح جديدة.
2. في Azure OpenAI Studio، في **صفحة عمليات التوزيع**، اعرض عمليات توزيع النموذج الحالية. إذا لم يكن لديك واحدة بالفعل، قم بإنشاء توزيع جديد لنموذج **gpt-35-turbo-16k** بالإعدادات التالية:
    - **النموذج**: gpt-35-turbo-16k *(إذا لم يكن نموذج 16k متوفراً، فاختر gpt-35-turbo)*
    - **إصدار النموذج**: Auto-update to default
    - **اسم التوزيع**: *اسم فريد من اختيارك. وستستخدم هذا الاسم لاحقاً في المختبر.*
    - **خيارات متقدمة**
...
1. في **Azure OpenAI Studio** في `https://oai.azure.com`، في **قسم Playground**، حدد صفحة **دردشة**. تتكون صفحة **دردشة** playground من ثلاثة أقسام رئيسية:
...
6. ضمن رسالة النظام الجديد، في قسم **الأمثلة**، حدد زر **إضافة**. ثم، أضف المثال التالي.
...
8. استخدم زر **تطبيق التغييرات** في أعلى قسم **إعداد** لتحديث رسالة النظام.
...
10. في قسم **إعداد**، قم بتغيير رسالة النظام مرة أخرى إلى القالب الافتراضي، والذي يجب أن يكون `You are an AI assistant that helps people find information.` بدون أمثلة. ثم، قم بتطبيق التغييرات.
...
> **تلميح**: إذا نسخت بالفعل مستودع **mslearn-openai**، فافتحه في تعليمات Visual Studio البرمجية. وإلا فاتبع هذه الخطوات لاستنساخه إلى بيئة تطويرك.
...
4. قم بتحديث قيم التكوين لتشمل:
    - **نقطة النهاية** و**مفتاح** من مورد Azure OpenAI الذي قمت بإنشائه (متوفر في صفحة **المفاتيح ونقطة النهاية** لمورد Azure OpenAI الخاص بك في مدخل Azure)
...
أنت الآن جاهز لاستخدام Azure OpenAI SDK لاستهلاك النموذج المنشور.
...
الآن بعد أن تم تكوين التطبيق الخاص بك، قم بتشغيله لإرسال طلبك إلى النموذج الخاص بك ومراقبة الاستجابة. ستلاحظ أن الفرق الوحيد بين الخيارات المختلفة هو محتوى المطالبة، وتظل جميع المعلمات الأخرى (مثل عدد الرموز المميزة ودرجة الحرارة) كما هي لكل طلب.

1. في مجلد اللغة المفضلة لديك، افتح `system.txt` في Visual Studio Code. لكل من التداخلات، ستقوم بإدخال **رسالة النظام** في هذا الملف وحفظها. سيتوقف كل تكرار مؤقتاً أولاً لتغيير رسالة النظام.
...
عند الانتهاء من مورد Azure OpenAI، تذكر حذف التوزيع أو المورد بأكمله في **مدخل Azure** في `https://portal.azure.com`.
